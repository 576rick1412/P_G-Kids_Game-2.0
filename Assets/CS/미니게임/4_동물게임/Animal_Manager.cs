...
public class Animal_Manager : MonoBehaviour
{
    ...
    public GameObject[] NullObject; //글자박스
    ...
    void Start()
    {
        ...
        for (int i = 0; i < OBJ_Num; i++) { target_POS[i] = NullObject[i].transform.position; }
    ...
    public void Drag_Animal(int i)
    {
        ...
        NullObject[i].transform.position = Camera.main.ScreenToWorldPoint(screenPoint);
        cheak[i] = false;
    ...
    public int Drop_Animal(int i)
    {
        ...
        {
            float Distance = Vector3.Distance(NullObject[i].transform.position, target[OBJ_order[j]].transform.position);
            if (Distance < Null_Num)
            {
                if (Null_cheak[j] == true) { NullObject[i].transform.position = target_POS[i]; sound.Wrong(); return 100; } //빈칸에 오브젝트가 있다면 리턴
                else
                {
                    if (i == j) // 올바른 칸일 경우 
                    {
                        NullObject[i].transform.position = target[OBJ_order[i]].transform.position; cheak[i] = true;
                        Null_cheak[j] = true;  sound.Right(); clear_cheak(); return j; }
                    else //칸은 맞으나 올바른 칸이 아닐경우
                    { NullObject[i].transform.position = target[OBJ_order[j]].transform.position; Null_cheak[j] = true; sound.Right(); return j; }
                }
            }
        }
        NullObject[i].transform.position = new Vector3(target_POS[i].x, target_POS[i].y, 100f); sound.Wrong(); return 100;
    }
    ...
    public void end_clear(){clear.SetActive(false);}
